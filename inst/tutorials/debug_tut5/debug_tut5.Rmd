...
The issue: The text you wanted to add with `xlab()` and `ylab()` was not put in parentheses. Therefore, `R` thought that "Body height (cm)" and "Body weight (kg)" would refer to some functions it is supposed to run --- but which obviously do not exist. 
...
You want to create a graph that shows the average ages of respondents, depending on whether or not they were born in Norway. The relevant variables are:
...
* Which geometric object you wanted to have in the graph: `geom_bar()` and...
